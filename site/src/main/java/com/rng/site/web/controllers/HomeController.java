...
{	
	...
	public String home(Model model)
	{
		...
		{
			Set<Test> test = category.getTests();
			List<Test> previewTests = new HashSet<>();
			int noOfProductsToDisplay = 4;
			if(test.size() > noOfProductsToDisplay){
				Iterator<Test> iterator = test.iterator();
				for (int i = 0; i < noOfProductsToDisplay; i++)
				{
					previewTests.add(iterator.next());
				}
			} else {
				previewTests.addAll(test);
			}	
